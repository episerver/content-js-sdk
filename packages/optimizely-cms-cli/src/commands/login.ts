...
export default class Login extends Command {
  static override description = 'Verify your Optimizely CMS credentials configured in environment variables';
  ...
  public async run(): Promise<void> {
    ...
    try {
      const token = await getToken(credentials.clientId, credentials.clientSecret, flags.host);
      ...
      if (token) {
        ...
        console.error(chalk.red('Authentication failed: No token received from the API'));
        process.exit(1);
      }
    } catch (err) {
