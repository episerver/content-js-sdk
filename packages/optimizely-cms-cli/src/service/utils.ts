...
async function compileAndImport(inputName: string, cwdUrl: string, outDir: string) {
 ...
 */
export function normalizePropertyGroups(propertyGroups: any[]): PropertyGroupType[] {
  ...
  if (duplicates.size > 0) {
    console.warn(
      chalk.yellow(
        `Warning: Duplicate property group keys found: ${Array.from(duplicates).join(
          ', ',
 ...
 */
export function extractKeyName(input: PermittedTypes, parentKey: string): string {
  return (
    typeof input === 'string' ?
      input === '_self' ?
        parentKey
