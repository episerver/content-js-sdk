...
If you need custom rendering for specific element types, use the `elements` or `leafs` props:
...
<RichText
  content={content.body?.json}
  elements={{
    'heading-one': ({ children }) => <h1 className="custom">{children}</h1>,
    'custom-widget': CustomWidget, // Your custom element
  }}
  leafs={{
